...
class AppRepository(private val context: Context) {
    ...
    suspend fun matchMemory(type: String, triggerWord: String): CategoryMemoryEntity? {
        return categoryMemoryDao.matchByTriggerWord(type, triggerWord)
    }
    ...
    /**
     * AccountingAiParser 记账解析：规则匹配失败时的 AI 兜底。
     * 只做记账解析，不聊天/不查询/不写库。
     * 返回 AccountingCandidate，由调用方决定是否展示确认卡片。
     */
    suspend fun parseWithAccountingAi(rawInput: String, requestId: String): com.accounting.app.ui.model.AccountingCandidate? {
        return accountingAiParser.parse(rawInput, requestId)
    }
...
如果用户输入的是记账类内容，请统一回复：「请到记账页记账」，不得编造任何记录结果、消费总额等虚假数据。
