 ...
 * 调用 DeepSeek AI 从预置分类列表（按收支方向区分支出/收入两套）中选择最合适分类，
 * 避免直接 fallback 到"其他"。
 *
 * 设计要点：
...
class AiClassifier(
     ...
     * @param type 收支方向（"expense"/"income"），决定使用哪套分类列表与默认分类
     ...
     */
    suspend fun correct(description: String, type: String, requestId: String, billIndex: Int?): MatchResult? {
        return try {
            // 1. 查缓存：命中且未过期则直接返回（缓存 key 带 type，避免收支方向不同的同描述串缓存）
            val cacheKey = "$type|$description"
            synchronized(cache) {
            ...
            // 3. 构建 Prompt：按收支方向列出对应分类列表，强约束输出
            val categoryList = CategoryConstants.getCategories(type).joinToString("、")
            val listLabel = if (type == "income") "收入" else "支出"
            val defaultName = if (type == "income") "其他收入" else "其他支出"
            val systemPrompt = """你是记账分类助手。请从以下${listLabel}分类列表中选择最合适的一个分类：
$categoryList

【严格规则】
1. 仅输出单个分类名，无标点、无解释、无换行、无 markdown
2. 必须从给定列表中选择，不能编造新分类
3. 无法判断时输出"$defaultName"
4. 直接输出分类名，不要有任何多余文字""".trimIndent()
            val userPrompt = "记账描述：$description"
            ...
            if (response.isSuccessful) {
                ...
                // 解析：trim 后精确匹配对应收支方向的分类列表
                val trimmed = content.trim()
                if (CategoryConstants.getCategories(type).contains(trimmed)) {
                    // 写入缓存（线程安全）
                    synchronized(cache) {
                        cache[cacheKey] = Pair(trimmed, System.currentTimeMillis())
                        // 缓存满时移除最旧条目
