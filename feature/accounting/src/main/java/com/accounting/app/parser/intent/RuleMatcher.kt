...
object RuleMatcher {
    ...
    private val incomeKeywords = listOf(
        ...
        // 与 builtinIncomeSceneMap 对齐的收入场景词，避免这些词被默认判为支出
        "薪水", "月薪", "年终奖", "绩效", "副业", "外快", "接单",
    ...
    fun match(
        ...
        requestId: String,
        billIndex: Int? = null,
        preJudgedType: String? = null
    ): MatchResult? {
        // 调用方已用完整文本做过收支方向判定时复用结果，避免重复判定与重复日志
        val type = preJudgedType ?: preJudgeType(request.description, requestId)
